...
contract OrderManagement {
        ...
        validStatus(_orderId, OrderStatus.Shipped)
    {
        ...
        if (!success) {
            revert("Failed to transfer payment to seller");
        }
    ...
    function emergencyCancel(uint256 _orderId, string memory _reason)
        external
        onlyOwner
        orderExists(_orderId)
    {
        ...
        require(bytes(_reason).length > 0, "Emergency cancellation reason cannot be empty");
        ...
        if (order.amount > 0) {
            (bool success, ) = order.buyer.call{value: order.amount}("");
            if (!success) {
                revert("Failed to refund buyer during emergency cancellation");
            }
        }
