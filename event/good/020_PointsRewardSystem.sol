...
contract PointsRewardSystem {
    ...
    modifier validAddress(address _address) {
        require(_address != address(0), "PointsRewardSystem: invalid address - zero address");
        _;
    }
        ...
        validAddress(recipient)
    {
        ...
        require(totalSupply + amount <= maxSupply, "PointsRewardSystem: award would exceed maximum supply");
    ...
    {
        require(amount > 0, "PointsRewardSystem: burn amount must be greater than zero");
        require(balanceOf[account] >= amount, "PointsRewardSystem: insufficient balance to burn");
        require(bytes(reason).length > 0, "PointsRewardSystem: reason cannot be empty");
    ...
    function setPaused(bool _paused) external onlyOwner {
        require(paused != _paused, "PointsRewardSystem: pause status already set to this value");
    ...
    function updateMaxSupply(uint256 _newMaxSupply) external onlyOwner {
        require(_newMaxSupply >= totalSupply, "PointsRewardSystem: new max supply cannot be less than current total supply");
        require(_newMaxSupply != maxSupply, "PointsRewardSystem: new max supply must be different from current");
    ...
    function renounceOwnership() external onlyOwner {
        revert("PointsRewardSystem: ownership cannot be renounced for security reasons");
    }
    ...
    function getContractInfo() external view returns (
        ...
        return (name, symbol, decimals, totalSupply, maxSupply, paused);
    }
