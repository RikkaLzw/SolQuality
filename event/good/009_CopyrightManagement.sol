...
contract CopyrightManagement {
        ...
        copyrightExists(_copyrightId)
    {
        ...
        uint256[] storage previousOwnerCopyrights = ownerCopyrights[previousOwner];
        for (uint256 i = 0; i < previousOwnerCopyrights.length; i++) {
            if (previousOwnerCopyrights[i] == _copyrightId) {
                previousOwnerCopyrights[i] = previousOwnerCopyrights[previousOwnerCopyrights.length - 1];
                previousOwnerCopyrights.pop();
                break;
            }
        }
    ...
    function grantLicense(
        ...
        uint256 _duration,
        uint256 _fee
    ) external payable onlyCopyrightOwner(_copyrightId) copyrightExists(_copyrightId) returns (uint256) {
        ...
        require(msg.value == _fee, "Sent value must match license fee");
        ...
        licenseExists(_licenseId)
    {
        ...
        require(
            copyrights[copyrightId].owner == msg.sender || msg.sender == admin,
    ...
    function withdrawLicenseFee(uint256 _licenseId)
        external
        licenseExists(_licenseId)
    {
        License storage license = licenses[_licenseId];
        ...
        require(copyrights[copyrightId].owner == msg.sender, "Only copyright owner can withdraw fees");
