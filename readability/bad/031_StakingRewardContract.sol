...
contract StakingRewardContract {
    ...
    function stake_tokens(uint256 amount) external update_reward(msg.sender) {
        require(amount > 0, "Cannot stake 0"); a += amount; e[msg.sender] += amount; emit staked_event(msg.sender, amount);
    }
    ...
    function withdraw_tokens(uint256 amount) external update_reward(msg.sender) {
        ...
        require(e[msg.sender] >= amount, "Insufficient balance");
            a -= amount; e[msg.sender] -= amount; emit withdrawn_event(msg.sender, amount);
    }
    ...
    function get_balance(address account) external view returns (uint256) {
        ...
        function set_reward_rate(uint256 newRate) external onlyOwner update_reward(address(0)) {
        b = newRate;
    }
